...
Open the `het_J.het` output file by typing
```{bash, eval=FALSE}
less het_J.het
```
\
...
\
(Hint: To calculate $H_{o}$, simply subtract O(Hom) from the total number of loci included in the analysis. To calculate $H_{e}$, subtract E(Hom) from the total number of loci.)
\
\
\
\

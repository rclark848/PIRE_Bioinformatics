Relatedness and Isolation-by-Distance Key
================
...
\
\
**J11-J13 relatedness is 0.0209635**
\
**J11-P1 relatedness is -0.0300251**
\
\
**Individual J11 is more slightly more related to J13 than to P1**
\
\
*Why do you think individuals are related with a value of 0.5 to themselves (e.g. INDV1 = J11 and INDV2 = J11 has a relatedness\_phi of 0.5)?*
\
*Hint: Think about what this relatedness score is really indicating.*
\
\
**This relatedness measure is the probability of choosing identical alleles when randomly sampling one allele from two individuals at a heterozygous loci. If comparing an individual to itself, the two heterozygous loci are the same, but there is still a 50% chance will pull the same allele (think of Mendelian inheritance from two heterozygote parents).**
\
\
...
<img src="Plots/relatedness.png" />

*How does relatedness vary with geographic distance?*
\
\
**Relatedness doesn't really change with geographic distance. There are a few more closely related individuals at a distance of 0 (although most of these are self-self comparisons). Overall, individuals are generally not very related to one another.**
\
\
*Does this fit an isolation-by-distance pattern? Why or why not?*
\
\
**It doesn't really fit an isolation-by-distance pattern. Relatedness slightly decays from a distance of 0, but there is no real change over most distance comparisons. This is likely because the study was not designed to test for IBD. The likelihood of finding full-siblings or parent/offspring matches at the same sampling location is low given the sampling size. If we don't have these matches, we are unlikely to see a decay pattern with distance (there is nothing to decay from).**
\
\
...
\
\
**r = 0.9429 --&gt; *F*<sub>*ST*</sub> and geographic distance are highly correlated.**
\
\
*Is the correlation statistically significant?*
\
\
**The significance value for the Mantel test was 0.16667, which is above the p-value cut-off of 0.05. Thus, this correlation is not statistically significant.**
\
\
*Please explain how you might change the study design to better test for isolation-by-distance patterns.*
\
\
**The low significance is likely because of the low power of the study. If we could collect more samples from within the geographic range analyzed (i.e. between Indonesia and Japan), we would be better able to test for patterns of isolation-by-distance.**

...
```
\
Then read in the Japan-Philippines $F_{ST}$ values that were output vy vcftools in our previous lab and save it to our new dataframe. Type:
```{r, eval=FALSE}
...
```
\
Now we can plot lienarized $F_{ST}$s vs. distance, with a best fit line added for good measure:
```{r, eval=FALSE}
